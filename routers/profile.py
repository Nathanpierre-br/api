...
async def check_in_history(
    ...
    start_dt = datetime.fromtimestamp(startTime / 1000, UTC) + timedelta(
        minutes=timezone
    )
...
async def get_user_wall_answers(
    uid: str, commentId: str, request: Request, ndcId: int = 0
):
    t1 = timestamp()
...
async def get_comment_voted_users(
    ...
    for item in votes_selected:
        ...
        voters_list.append(User.GetUserInfo(user, ndcId=ndcId))
